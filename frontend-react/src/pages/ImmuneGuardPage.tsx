...
const pdfValue = (value: unknown) => textValue(value)
    .replace(/[–—]/g, '-')
    .replace(/\s+/g, ' ')
    .trim();
...
const ImmuneGuardPage = () => {
    ...
    const handleExportPDF = () => {
        if (!riskResult) {
            addToast('Aucun resultat ImmuneGuard a exporter.', 'error');
            return;
        }

        const exportedAt = new Date();
        const exportStamp = formatExportDate(exportedAt);
        const doc = new jsPDF({ orientation: 'landscape' });
        const pageWidth = doc.internal.pageSize.getWidth();
        const sid = riskResult.score_i_details as Record<string, unknown> | null | undefined;
        const incidents = Array.isArray(sid?.incidents) ? (sid.incidents as Record<string, unknown>[]) : [];
        ...
        doc.text('ImmuneGuard Score Report', 14, 18);
        doc.setFontSize(10);
        doc.text(`Dataset: ${pdfValue(currentDatasetName)}`, 14, 28);
        doc.text(`Export: ${exportedAt.toLocaleString('fr-FR')}`, 14, 35);

        doc.setTextColor(15, 23, 42);
        doc.setFontSize(14);
        doc.text('Synthèse du risque', 14, 52);

        autoTable(doc, {
            startY: 58,
            head: [['Indicateur', 'Valeur']],
            body: [
                ['Dataset ID', pdfValue(selectedId)],
                ['Classification', pdfValue(riskResult.classification)],
                ['R final', `${Math.round(riskResult.R_final)}%`],
                ['R base', `${(riskResult.R_base * 100).toFixed(1)}%`],
                ['B - Risque intrinsèque', pct(riskResult.B)],
                ['A - Exploitabilité adversariale', pct(riskResult.A)],
                ['I - Incidents documentés', pct(riskResult.I)],
                ['C - Facteur contextuel', riskResult.C.toFixed(2)],
            ],
        ...
        autoTable(doc, {
            ...
            head: [['Champ DataCard', 'Valeur']],
            body: [
                ['Secteur', dataCard.secteur || analysisProposal?.secteur],
                ['Finalité', dataCard.finalite],
                ['Population', dataCard.population],
                ['Pays', dataCard.pays],
                ['Origine', dataCard.origine],
                ['Information', dataCard.information],
            ].map(([label, value]) => [pdfValue(label), pdfValue(value)]),
        ...
        autoTable(doc, {
            ...
            head: [['Score B', 'Valeur']],
            body: [
                ['b1 - Exposition directe', pct(riskResult.score_b_details.b1)],
                ['b2 - Gravité réglementaire', pct(riskResult.score_b_details.b2)],
                ['b3 - Densité des signaux', pct(riskResult.score_b_details.b3)],
                ['b4 - Qualité brute', pct(riskResult.score_b_details.b4)],
                ['B agrégé', pct(riskResult.B)],
            ],
        ...
        autoTable(doc, {
            ...
            head: [['Score I', 'Valeur']],
            body: [
                ['Secteur I', pdfValue(sid?.secteur ?? scoreIResult?.secteur)],
                ['Colonnes à signal', pdfValue(sid?.n_signaux)],
                ['Similarité moyenne', typeof sid?.sim_moyen === 'number' ? pct(sid.sim_moyen) : '-'],
                ['Sévérité moyenne', typeof sid?.grav_moyen === 'number' ? pct(sid.grav_moyen) : '-'],
                ['Domaines MIT actifs', pdfValue(domaines)],
                ['I agrégé', pct(riskResult.I)],
            ],
        ...
        if (scoreResult) {
            autoTable(doc, {
                ...
                head: [['Score A', 'Valeur']],
                body: [
                    ['Singling out', pct(scoreResult.taux_singling)],
                    ['Linkage attack', pct(scoreResult.taux_linkage)],
                    ['Inference attribut', pct(scoreResult.taux_inference)],
                    ['Succ attaques', pct(scoreResult.Succ)],
                    ['Vuln controles', pct(scoreResult.Vuln)],
                    ['A agrégé', pct(scoreResult.A)],
                ],
                headStyles: { fillColor: [139, 92, 246] },
            });
        }

        if (datasetScanContext.detected.length > 0) {
            autoTable(doc, {
                startY: (doc as any).lastAutoTable.finalY + 10,
                head: [['Colonne', 'Signal', 'Domaine MIT', 'Source']],
                body: datasetScanContext.detected.map((col) => [
                    pdfValue(col.raw_column),
                    shortSignalLabel(col.signal),
                    pdfValue(col.mit_domain),
                    pdfValue(col.legal_source),                ]),
        ...
        if (incidents.length > 0) {
            autoTable(doc, {
                startY: (doc as any).lastAutoTable.finalY + 10,
                head: [['Incident', 'Similarité', 'Sévérité']],
                body: incidents.map((inc, idx) => [
                    pdfValue(inc.title ?? inc.titre ?? `Incident ${idx + 1}`),
                    typeof inc.sim === 'number' ? inc.sim.toFixed(2) : '-',
                    typeof inc.grav === 'number' ? inc.grav.toFixed(2) : '-',
                ]),
                headStyles: { fillColor: [217, 119, 6] },
            });
        }

        if (riskResult.xai.length > 0) {
            autoTable(doc, {
                startY: (doc as any).lastAutoTable.finalY + 10,
                head: [['Colonne / signal', 'Sévérité', 'Source légale', 'Explication']],
                body: riskResult.xai.map((item) => [
                    pdfValue(item.column || item.signal),
                    `${item.severity}/5`,
                    pdfValue(item.legal_source),
                    pdfValue(item.explanation),
                ]),
                styles: {
                    cellWidth: 'wrap',
                    cellPadding: 2,
        ...
        doc.save(`immuneguard_score_${cleanFilePart(currentDatasetName)}_${exportStamp}.pdf`);
        addToast('Export ImmuneGuard PDF généré.', 'success');
    };
            ...
            <div className="flex flex-col xl:flex-row justify-between items-start xl:items-end gap-6">
                ...
                <div className="flex gap-3 w-full xl:w-auto">
                    ...
                    <Button
                        ...
                        disabled={!riskResult}
                        title={riskResult ? 'Exporter le rapport ImmuneGuard en PDF' : 'Lancez le calcul ImmuneGuard Score avant export'}
                        className={`justify-center ${riskResult ? 'cursor-pointer' : 'opacity-50 cursor-not-allowed'}`}
                    >
